...
app.get("/trucks", async (req, res) => {
  pool.query(
    "SELECT * FROM trucks JOIN locations ON locations.trucks_id = trucks.id WHERE locations.day = 'monday';",
    function (err, result) {
      if (err) {
...
app.get("/trucks/:day", async (req, res) => {
  ...
  pool.query(
    `SELECT * FROM trucks JOIN locations ON locations.trucks_id = trucks.id WHERE locations.day = '${day}';`,
    function (err, result) {
      if (err) {
        return console.error("error running query", err);
      }
...
app.get("/trucks/:day/:filter", async (req, res) => {
  ...
  console.log("DAY:", day);
  pool.query(
    `SELECT * FROM trucks JOIN locations ON locations.trucks_id = trucks.id WHERE info LIKE '%${keyword}%' AND locations.day= '${day}';`,
    function (err, result) {
      if (err) {
        return console.error("error running query", err);
      }
...
app.post("/signup", async (req, res) => {
  try {
    ...
    let result = await pool.query(insertTrucks, [
      req.body.name,
      req.body.info,
      5,
    ]);
    ...
    let insertLocationsValues1 = [
      result.rows[0].id,
      req.body.dayLocation[1].lat,
      req.body.dayLocation[1].lng,
      "monday",
    ];
    let insertLocationsValues2 = [
      result.rows[0].id,
      req.body.dayLocation[2].lat,
      req.body.dayLocation[2].lng,
      "tuesday",
    ];
    let insertLocationsValues3 = [
      result.rows[0].id,
      req.body.dayLocation[3].lat,
      req.body.dayLocation[3].lng,
      "wednesday",
    ];
    let insertLocationsValues4 = [
      result.rows[0].id,
      req.body.dayLocation[4].lat,
      req.body.dayLocation[4].lng,
      "thursday",
    ];
    let insertLocationsValues5 = [
      result.rows[0].id,
      req.body.dayLocation[5].lat,
      req.body.dayLocation[5].lng,
      "friday",
    ];
    let insertLocationsValues6 = [
      result.rows[0].id,
      req.body.dayLocation[6].lat,
      req.body.dayLocation[6].lng,
      "saturday",
    ];
    let insertLocationsValues7 = [
      result.rows[0].id,
      req.body.dayLocation[7].lat,
      req.body.dayLocation[7].lng,
      "sunday",
    ];
